...
namespace SystemRezerwacjiKortow.Database
{
    public static class SqlUser
    {
        ...
        {
            SqlConnection connection = SqlDatabase.NewConnection();
        ...
        public static bool CheckUserExists(string email)
        {
            ...
            if (SqlDatabase.OpenConnection(connection))
            {
                var command = new SqlCommand("SELECT COUNT(*) FROM [User] WHERE Email = @email", connection);
        ...
        public static bool FindActivationCode(string code)
        {
            ...
            if (SqlDatabase.OpenConnection(connection))
            {
                var command = new SqlCommand("SELECT COUNT(*) FROM [User] WHERE ActivationCode = @code", connection);
        ...
        public static bool CheckEmailVeryfied(UserLogin login)
        {
            ...
            if (SqlDatabase.OpenConnection(connection))
            {
                var command = new SqlCommand("SELECT * FROM [User] WHERE Email = @email and IsEmailVeryfied = 1", connection);
        ...
        public static void UpdateUserEmailVeryfied(string code)
        {
            ...
            if (SqlDatabase.OpenConnection(connection))
            {
                var command = new SqlCommand("UPDATE [User] SET IsEmailVeryfied = 1  WHERE ActivationCode = @code", connection);
                command.Parameters.AddWithValue("@code", code);
        ...
        public static string GetUserPassword(UserLogin login)
        {
            ...
            if (SqlDatabase.OpenConnection(connection))
            {
                var command = new SqlCommand("SELECT Password FROM [User] WHERE Email = @email", connection);
                command.Parameters.AddWithValue("@email", login.Email);
